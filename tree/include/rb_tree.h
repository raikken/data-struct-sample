...
class NodeIterator{
    public:
        ...
        NodeIterator(pointer val):val(pointer){};
...
class RbTree {
    public:
        ...
        //insert_rebalance
        void insert_rebalance(node_pointer xptr, node_pointer xparentptr);
    ...
    while(xptr != this->root && xptr->parent->color == RED) {
        ...
        if (xparentptr->parent->left == xparentptr) {
            ...
            if (xuncleptr && xuncleptr->color == RED){
                xparentptr->color = BLACK;
                xuncleptr->color = BLACK;
                xparentpt->parent->color = RED;
                xptr = xparentpt->parent;
            } else {
        
                node = // 叔父节点不存在, 或者叔父节点为黑
...
void delete_rebalance(node_ptr xptr) {
    ...
    while(xptr != this->root || xptr->color == BLACK) {
        ...
        if (xparentptr->left == xptr) {
           ...
           //对称的逻辑, 
           xbrotherptr = xparentptr->left;
           //case1
           if (xbrotherptr->color == RED) {
               xbrotherptr->color = BLACK;
               xparentptr->color = RED;
               rotate_right(xparentptr);
           } eles if (xbrotherptr->color == BLACK && xbrotherptr->left->color == BLACK && xbrotherptr->right->color == BLACK) {
